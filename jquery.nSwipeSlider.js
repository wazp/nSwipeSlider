...
; (function ($) {
	$.fn.nSwipeSlider = function (userOptions) {
		...
		var defaults = { wrapAround: true, nonTransSpeed: 200, destroy: false }, // default options. nonTransSpeed is anim milliseconds for non css transition browsers
		...
		return this.each(function () {
			...
			if (alreadyApplied) return; // if already loaded, it won't be undefined.
			$this.data('nSwipeSlider', { loaded: true, wrapped: options.wrapAround }); // add data to element that we've already loaded a slider on it.
			if (!$this.find('.active').length) $this.addClass('notransition').children().eq(0).addClass('active');
			if (options.wrapAround) { // if we want a wrap around effect...
				$this.children().eq(0).clone(true).appendTo($this).removeClass('active'); // ...we need to clone elements from first to last, last to first to simulate it.
			...
			$items.on('movestart', function (e) {
				$wrap.addClass('notransition').children().removeClass('notransition'); // turn off the css3 transitions for now
				...
				if (hasTrans) {
					...
					$items.eq(i).prev().animate({ left: '-100%' }, options.nonTransSpeed);
					$items.eq(i).next().animate({ left: '100%' }, options.nonTransSpeed);
				}
			...
			function changeCurrent() { // this takes care of the wrap around when we're using built in css3 transitions!
			...
			if (options.wrapAround && hasTrans) { // again, if we're using build in css3 transitions, bind events to listen to transEnd events on last items
